...
namespace TradeOrganizer_Group17_DIS_Fall_2018
{
    public partial class StockList
    {
        ...
        {
            //variable to store size of linked list
            int size;

            //store length of linked list
            size = Length();

            //if size is greater than 1 then continue
            if (size > 1)
            {
                //outer loop to traverse linked list
                for (int i = 0; i < size; i++)
                {
                    //assign current and next nodes to StockNode objects
                    StockNode currentNode = head;
                    StockNode next = head.Next;

                    //inner loop to compare Holdings in each Stock
                    for (int j = 0; j < size - 1; j++)
                    {
                        //compare Holdings of current and next Stock node. If holdings of current node
                        // is less than next node Holdings then Swap.
                        if (currentNode.StockHolding.Holdings < next.StockHolding.Holdings)
                        {
                            //Swap node
        ...
        public void SortByName()
        {
            ...
            if (size > 1)
            {
                ...
                for (int i = 0; i < size; i++)
                {
                    ...
                    //inner loop to compare Name in each Stock
                    for (int j = 0; j < size - 1; j++)
                    {
                        //compare Names of current and next node. It will return an integer value
                        int cmpVal = currentNode.StockHolding.Name.CompareTo(next.StockHolding.Name);

                        //if Integer value returned is greater than 0 then Swap nodes
                        if (cmpVal > 0)
                        {
                            //Swap
                            Swap(currentNode.StockHolding);
